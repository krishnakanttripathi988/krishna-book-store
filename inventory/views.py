...
from .searchModule import gbooks
import re, json, requests
from django.utils import timezone
from .prebuilt import start1
import os
...
class IndexView(TemplateView):
	...
	def post(self, request):
		...
		try:
			...
			return render(request, 'inventory/search.html', {'b':b})
...
class BooksUpdate(TemplateView):
	...
	def post(self, request, ID):
		...
		if updateform.is_valid():
			...
			q.save()
		books= Books.objects.all()
		return render(request, 'inventory/inventory', {'books': books})
...
class BooksDelete(TemplateView):
	...
	def get(self, request, ID):
		...
		q.delete()
		books=Books.objects.all()
		return render(request, 'inventory/inventory.html', {'books': books})

class BooksAdd(TemplateView):
	def get(self, request):
		addform= AddForm
		return render(request, 'inventory/add.html', {'form':addform})
   ...
   #http://books.google.com/books?id=7ep09WAFbDwC&printsec=frontcover&dq=love&hl=&cd=1&source=gbs_api
			temp_key=key[0][3:-1]
		googleapikey="AIzaSyDaZHVRXeYJzcyPvByaWYkmElLEECV8TGU"
